...
public:
    ...
    long long maxPoints(vector<vector<int>>& point) {
        int n=point.size();
        int m=point[0].size();
        vector<vector<long long int>>dp(n,vector<long long int>(m,-1e16));
        ...
        for(int i=1;i<n;i++)
        {
            ...
            for(int j=0;j<m;j++)
            {
                left[j+1]=max(dp[i-1][j]+j,left[j]);
               // maxil=max(maxil,left[j]);
            }
            for(int j=m-1;j>=0;j--)
            {
                right[j]=max(dp[i-1][j]-j,right[j+1]);
               // maxir=max(maxir,right[j]);
            }
            for(int j=0;j<m;j++)
            {
                dp[i][j]=max({dp[i][j],point[i][j]+left[j+1]-j,point[i][j]+right[j+1]+j});
                if(i==n-1)
                {
                    ans=max(ans,dp[i][j]);
                }
